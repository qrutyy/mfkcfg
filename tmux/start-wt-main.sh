...
# 0.1 - left-down
tmux send-keys -t $SESSION:main.2 "sudo su" C-m
tmux send-keys -t $SESSION:main.2 "dmesg -C" C-m
tmux send-keys -t $SESSION:main.2 "dmesg -w" C-m

# 0.2 right
tmux send-keys -t $SESSION:main.3 "sudo su" C-m
tmux send-keys -t $SESSION:main.3 "source ~/.bash_profile" C-m
